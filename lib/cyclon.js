 ...
 * @returns {Function}
 */
function addToPartialView(peer, callback) {
    return function () {
 ...
 * @returns {Function}
 */
exports.bootstrap = function(options, signaling, callback){
    var onOffer = Utils.isDefined(signaling.onOffer) ? signaling.onOffer : null;
    var onAnswer = Utils.isDefined(signaling.onAnswer) ? signaling.onAnswer : null;
    var offer = Utils.isDefined(signaling.offer) ? signaling.offer : null;
    ...
    partialView = []; // reset
    var peer, timeout = setTimeout(function () {
    ...
    if (onOffer !== null) {
        ...
        peer.onopen(addToPartialView(peer, callback));

        return function (answer) {
            clearTimeout(timeout);
            Handshake.handleAnswer(peer, answer);
        };
    } else if (onAnswer !== null && offer !== null){
        // ==================================
        // O N  A N S W E R
        // ==================================
        peer = Handshake.createAnswer(offer, onAnswer);
        peer.onopen(function () {
            clearTimeout(timeout);
            addToPartialView(peer, callback)();
        });
