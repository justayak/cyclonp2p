...
function shuffle(cyclon) {
    setInterval(function () {
        var Q, partialViewNoQ, partialView = cyclon.partialView;
        if (partialView.length > 0) {
            // #1 select random subset 'l' and a random peer Q
            Q = Protocol.sample(partialView,1);
        }
    }, cyclon.delta_t);
}
...
/**
 * Helper function for applying onload
 * @param cyclon
 */
function ready(cyclon) {
    return function () {
        cyclon.isReady = true;
        var i = 0, L = cyclon.partialView.length;
        for(;i<L;i++) {
            cyclon.partialView[i].call(cyclon);
        }
